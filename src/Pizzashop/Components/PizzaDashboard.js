...
const PizzaDashboard = () => {
  ...
  return (
    <div className='container'>
              ...
              <div className="col-sm-3 PlaceOrder orderdiv">
                ...
                {orders
                  ...
                  .map((order) => (
                    <div
                      key={order.id}
                      className={`order-item ${shouldHighlight('Order Placed', order.size, timers[order.id + 'Order Placed'])}`}
                    >
                      <p>
                        Order ID: {order.id} - Stage: {order.stage} - Remaining Time:{' '}
                        {formatTime(timers[order.id + order.stage])}
                      </p>
                      <button onClick={() => handleMoveOrder(order.id, 'Order in Making')}>Next</button>
                    </div>
                  ))}
              </div>
              ...
              <div className="col-sm-3 MakingOrder orderdiv">
                ...
                {orders
                  ...
                  .map((order) => (
                    <div
                      key={order.id}
                      className={`order-item ${shouldHighlight('Order in Making', order.size, timers[order.id + 'Order in Making'])}`}
                    >
                      <p>
                        Order ID: {order.id} - Stage: {order.stage} - Remaining Time:{' '}
                        {formatTime(timers[order.id + order.stage])}
                      </p>
                      <button onClick={() => handleMoveOrder(order.id, 'Order Ready')}>Next</button>
                    </div>
                  ))}
              </div>
              ...
              <div className="col-sm-3 ReadyOrder orderdiv">
                ...
                {orders
                  ...
                  .map((order) => (
                    <div
                      key={order.id}
                      className={`order-item ${shouldHighlight('Order Ready', order.size, timers[order.id + 'Order Ready'])}`}
                    >
                      <p>
                        Order ID: {order.id} - Stage: {order.stage} - Remaining Time:{' '}
                        {formatTime(timers[order.id + order.stage])}
                      </p>
                      <button onClick={() => handleMoveOrder(order.id, 'Order Picked')}>Next</button>
                    </div>
                  ))}
              </div>
                ...
                {orders
                  ...
                  .map((order) => (
                    <div key={order.id}>
                      ...
                      {/* Only 'Picked' button, as per the instructions */}
                      <p>Picked</p>
                    </div>
                  ))}
              </div>
      ...
      <div>
        <p>Total Pizzas Delivered Today: {orders.filter((order) => order.stage === 'Order Picked').length.toString().padStart(3, '0')}</p>
      </div>
    </div>
  );
};
...
//   const canTakeOrder = orders.length < MAX_ORDERS;
...
//       (totalTime, stage) => totalTime + (timers[orderId + stage] || 0),
//       0
//     );
//   };
...
//     switch (stage) {
//       case 'Order Placed':
//         makingTime = size === 'Small' ? 30 : (size === 'Medium' ? 40 : 50);
//         break;
//       case 'Order in Making':
//         makingTime = size === 'Small' ? 30 : (size === 'Medium' ? 40 : 50);
//         break;
...
//                   .filter((order) => order.stage === 'Order Placed')
//                   .map((order) => (
//                     <div key={order.id}  className={`order-item ${shouldHighlight('Order Placed', order.size, timers[order.id + 'Order Placed'])}`}>
//                       <p>
//                         Order ID: {order.id} - Stage: {order.stage} - Remaining Time:{' '}
//                         {formatTime(timers[order.id + order.stage])}
//                       </p>
...
//                   .filter((order) => order.stage === 'Order in Making')
//                   .map((order) => (
//                     <div key={order.id} className={`order-item ${shouldHighlight('Order in Making', order.size, timers[order.id + 'Order in Making'])}`}>
//                       <p>
//                         Order ID: {order.id} - Stage: {order.stage} - Remaining Time:{' '}
//                         {formatTime(timers[order.id + order.stage])}
//                       </p>
...
//                   .filter((order) => order.stage === 'Order Ready')
//                   .map((order) => (
//                     <div key={order.id} className={`order-item ${shouldHighlight('Order Ready', order.size, timers[order.id + 'Order Ready'])}`}>
//                       <p>
//                         Order ID: {order.id} - Stage: {order.stage} - Remaining Time:{' '}
//                         {formatTime(timers[order.id + order.stage])}
//                       </p>
//                       {/* Only 'Next' button, as per the instructions */}
//                       <button onClick={() => handleMoveOrder(order.id, 'Order Picked')}>Next</button>
//                     </div>
//                   ))}
//               </div>
